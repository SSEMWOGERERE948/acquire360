...
function TeamCard({ member }: { member: TeamMember }) { return <article data-testid={`card-team-${member.id}`} className="border border-[hsl(var(--border))] bg-[hsl(var(--card))]"><div className="image-wash flex h-48 items-end bg-[hsl(var(--primary)/.12)] p-5">{member.image ? <img src={member.image} alt={member.name} className="h-full w-full object-cover" /> : <span className="text-5xl font-bold text-[hsl(var(--primary)/.2)]">{member.name.split(' ').map(s => s[0]).join('').slice(0, 2)}</span>}</div><div className="p-6"><h3 className="font-bold text-[hsl(var(--primary))]">{member.name}</h3><p className="mt-1 text-xs font-bold uppercase tracking-widest text-[hsl(var(--secondary))]">{member.position}</p><p className="mt-4 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{member.bio}</p></div></article>; }
